...
## 2026-05-29 - API Path Traversal via Unencoded Parameters
**Vulnerability:** The SmugMug API Client constructed REST API endpoints using string interpolation without URL-encoding the parameters (e.g., `nickname`, `album_key`, `image_key`). An attacker controlling these parameters could inject path traversal sequences (like `../`) or arbitrary query parameters to manipulate the requested endpoint, potentially leading to unauthorized data access or SSRF via endpoint injection.
**Learning:** Never construct API endpoints or URLs using direct string interpolation with untrusted or externally sourced parameters. All variable parts of an endpoint path must be properly URL-encoded.
**Prevention:** Use `urllib.parse.quote(param, safe="")` on all path parameters before inserting them into an endpoint string to ensure characters like `/`, `?`, and `.` are safely encoded.

## 2026-05-29 - TOCTOU Vulnerability in File Existence Checks
**Vulnerability:** The `download_image_worker` function in `src/downloader.py` used `os.path.exists()` to check if a file already existed before calling `os.path.getsize()` and `verify_md5()` on it. This introduced a Time-Of-Check to Time-Of-Use (TOCTOU) vulnerability where the file could be modified or deleted between the existence check and subsequent file operations.
**Learning:** Using `os.path.exists()` before performing file operations creates a race condition window. The file system state can change concurrently, leading to errors or security issues if the operations assume the file state remained exactly as it was during the check.
**Prevention:** Always use the "Easier to Ask for Forgiveness than Permission" (EAFP) pattern for file operations. Instead of checking for existence beforehand, attempt the operation (e.g., `os.path.getsize`, `open`) inside a `try...except OSError` block. This ensures that the existence check and the file operation are handled atomically.

## 2026-09-04 - Rate Limit and Server Error Stack Trace Leakage Fix
