...
## 2024-07-22 - Premature Escaping in Internal Errors
**Vulnerability:** The application was applying `rich.markup.escape()` directly when raising internal exceptions (like `SmugMugAPIError`). While this prevents terminal output injection, it prematurely formats data intended for internal system usage, leading to corrupted error strings if they are logged, processed, or displayed by a non-rich frontend.
**Learning:** Security sanitization should only occur at the exact boundary where the data leaves the application (e.g., right before `console.print()`). Modifying internal state or exception payloads with presentation-layer escaping violates separation of concerns and can cause downstream data integrity issues.
**Prevention:** Apply output escaping (like `escape()`) exclusively at the presentation layer. Ensure that internal exceptions and logs retain the raw, original data format.
