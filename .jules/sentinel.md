...
## 2024-05-28 - API Path Traversal and Endpoint Injection
**Vulnerability:** The API client used string formatting (e.g., `f"{API_ROOT}/user/{nickname}!albums"`) to construct endpoints with user-controlled or dynamically fetched inputs like `nickname`, `album_key`, and `image_key` without sanitizing them. This allowed an attacker to inject path traversal characters (like `../`) or other control characters to access unauthorized endpoints or alter the structure of the resulting API request.
**Learning:** Any dynamic values inserted into a URL path must be strictly URL-encoded to prevent malicious traversal or endpoint injection. A malicious API key or nickname containing `../` could trick the backend or client into accessing the wrong URI.
**Prevention:** Always URL-encode external or untrusted parameters using `urllib.parse.quote(param, safe='')` before interpolating them into the endpoint string. This ensures slashes and control characters are safely escaped as their percent-encoded equivalents.
